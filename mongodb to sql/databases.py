...
def database_exist(dbname, dbuser, dbpassword):
	...
	try:
		sql_db = psycopg2.connect("dbname={} user={} password={}".format(dbname, dbuser, dbpassword)) # connect to database
		return 1
	except:
		return 0
...
def drop_create_database(dbname, dbrootname, dbuser, dbpassword):
	...
	sql_db = psycopg2.connect("dbname={} user={} password={}".format(dbrootname, dbuser, dbpassword)) # connect to database
	cur = sql_db.cursor()
